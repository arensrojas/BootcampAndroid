...
/*
En base al modelo anteriormente planteado realice las siguientes operaciones:
...
*/
SELECT nombre AS 'nombre del fabricante', UPPER(LEFT(nombre, 2)) AS 'abreviatura' FROM fabricante;
...
tabla productos.
*/
SELECT COUNT(DISTINCT id_fabricante) AS total_fabricantes_distintos FROM producto;

/*
4. Calcula la media del precio de todos los productos.
*/
SELECT AVG(precio) AS media_precios FROM producto;
...
*/
SELECT fabricante.nombre, COUNT(producto.id) AS total_productos
FROM fabricante
...
*/
SELECT fabricante.nombre, MAX(producto.precio) AS precio_maximo, MIN(producto.precio) AS precio_minimo, AVG(producto.precio) AS precio_medio
FROM fabricante
LEFT JOIN producto ON fabricante.id = producto.id_fabricante
GROUP BY fabricante.id;
...
*/
SELECT id_fabricante, MAX(precio) AS precio_maximo, MIN(precio) AS precio_minimo, AVG(precio) AS precio_medio, COUNT(*) AS numero_total
FROM producto GROUP BY id_fabricante HAVING precio_medio > 200;
...
*/
SELECT f.nombre AS nombre_fabricante, MAX(p.precio) AS precio_maximo, MIN(p.precio) AS precio_minimo, AVG(p.precio) AS precio_medio, COUNT(*) AS numero_total
FROM producto p
...
*/
SELECT COUNT(*) AS numero_productos FROM producto WHERE precio >= 180;
...
fabricante.
*/
SELECT p.id_fabricante, AVG(p.precio) as precio_medio
FROM producto p
